...
class TestFlightSearchFunctions(unittest.TestCase):
    ...
    @patch('requests.get')
    def test_search_flight_destinations_success(self, mock_get, mock_create_auth_headers):
        mock_response = Mock()
        mock_response.json.return_value = {'data': [{'destination': 'LON', 'departureDate': '2024-07-30', 'price': {'total': '200'}}]}
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
        mock_create_auth_headers.return_value = {'Authorization': 'Bearer abc123'}

        result = search_flight_destinations(self.origin, self.departure_date, self.one_way, self.non_stop, self.max_price)
        self.assertIsNotNone(result)
        self.assertIn('data', result)

    @patch('flight_search.create_auth_headers')
    @patch('requests.get')
    def test_search_flight_destinations_http_error(self, mock_get, mock_create_auth_headers):
        mock_response = Mock()
    ...
    def test_find_unique_destinations_success(self, mock_extract_flight_information, mock_search_flight_destinations):
        mock_search_flight_destinations.return_value = {'data': [
            {'destination': {'iataCode': 'LON', 'name': 'London'}},
            {'destination': {'iataCode': 'NYC', 'name': 'New York'}}
        ]}

        mock_extract_flight_information.side_effect = [
            {'destination': 'LON', 'departure_date': '2024-07-30', 'return_date': '2024-08-06', 'price_total': '200'},
            {'destination': 'NYC', 'departure_date': '2024-07-30', 'return_date': '2024-08-06', 'price_total': '400'}
        ]

        result = find_unique_destinations(self.origin, self.departure_date, self.one_way, self.non_stop, self.max_price)
